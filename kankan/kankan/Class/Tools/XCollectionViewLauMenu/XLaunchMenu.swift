...
protocol XLaunchMenuDelegate: NSObjectProtocol {
    
    func itemPressedWithIndex(index:Int)
}

class XLaunchMenu: UIViewController, UICollectionViewDataSource, UICollectionViewDelegate {
     ...
     */
    weak var delegate:XLaunchMenuDelegate?
    ...
    private func createLaunchMenu() {
        ...
        launchMenu = UICollectionView(frame: CGRectMake(0, scNavBarHeight, kScreenWidth, height), collectionViewLayout: layout)
        launchMenu.delegate = self
        launchMenu.dataSource = self
        launchMenu.backgroundColor = UIColor.whiteColor()
        launchMenu.registerClass(XLaunchMenuItem.self, forCellWithReuseIdentifier: "ITEM")
    ...
    func collectionView(collectionView: UICollectionView, cellForItemAtIndexPath indexPath: NSIndexPath) -> UICollectionViewCell {
        let cell = collectionView.dequeueReusableCellWithReuseIdentifier("ITEM", forIndexPath: indexPath) as! XLaunchMenuItem
