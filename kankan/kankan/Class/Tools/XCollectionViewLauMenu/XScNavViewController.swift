...
class XScNavViewController: UIViewController, XScNavBarDelegate, UIScrollViewDelegate {
    ...
    private var scNavBar:XScNavBar!    //导航栏视图
     ...
     */
    init(subViewControllers:NSArray, parentViewController:HotViewController, show:Bool) {
        super.init(nibName: nil, bundle: nil)
        self.subViewControllers = subViewControllers
        self.showArrowButton = show
        self.addParentController(parentViewController)
...
//        viewcontroller.edgesForExtendedLayout = UIRectEdge.None
    ...
    private func initParamConfig() {
        //初始化一些变量
        currentIndex = 1
        scNavBarColor = scNavBarColor != nil ? scNavBarColor : navColor
    ...
    private func initWithScNavBarAndMainView() {
        scNavBar = XScNavBar(frame: CGRectMake(0, 64, kScreenWidth, scNavBarHeight), show: showArrowButton, image: scNavBarArrowImage)
    ...
    func isShowScNavBarItemMenu(show: Bool, height: CGFloat) {
        if show {
            ...
            UIView.animateWithDuration(0.5, animations: { () -> Void in
                self.scNavBar.frame = CGRectMake(self.scNavBar.frame.origin.x, self.scNavBar.frame.origin.y, kScreenWidth, scNavBarHeight)
            })
        }
        scNavBar.refreshAll()
    }
