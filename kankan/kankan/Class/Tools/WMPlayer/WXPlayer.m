...
#define WXPlayerFrameworkSrcName(file) [@"Frameworks/WXPlayer.framework/WXPlayer.bundle" stringByAppendingPathComponent:file]
#define xHalfWidth self.frame.size.width * 0.5
#define xHalfHeight self.frame.size.height * 0.5
...
@interface WXPlayer () <UIGestureRecognizerDelegate>
...
/**
 *  初始化WXPlayer的控件，添加手势，添加通知，添加kvo等
 */
-(void)initWXPlayer{
    self.seekTime = 0.00;
...
//        
//    }];
    self.topView.frame = CGRectMake(0, 0, xScreenWidth, 40);
...
//        
//    }];
    self.bottomView.frame = CGRectMake(0, self.frame.size.height-40, xScreenWidth, 40);
    ...
    [self.playOrPauseBtn setImage:[UIImage imageNamed:WXPlayerSrcName(@"pause")] ?: [UIImage imageNamed:WXPlayerFrameworkSrcName(@"pause")] forState:UIControlStateNormal];
    [self.playOrPauseBtn setImage:[UIImage imageNamed:WXPlayerSrcName(@"play")] ?: [UIImage imageNamed:WXPlayerFrameworkSrcName(@"play")] forState:UIControlStateSelected];
...
//    }];
    ...
    [self.progressSlider setThumbImage:[UIImage imageNamed:WXPlayerSrcName(@"dot")] ?: [UIImage imageNamed:WXPlayerFrameworkSrcName(@"dot")]  forState:UIControlStateNormal];
...
//    }];
    self.progressSlider.frame = CGRectMake(44, 1, xScreenWidth-90, 40);
...
//    }];
    self.loadingProgress.frame = CGRectMake(45, 20, xScreenWidth-90, 40);
    ...
    [self.fullScreenBtn setImage:[UIImage imageNamed:WXPlayerSrcName(@"fullscreen")] ?: [UIImage imageNamed:WXPlayerFrameworkSrcName(@"fullscreen")] forState:UIControlStateNormal];
    [self.fullScreenBtn setImage:[UIImage imageNamed:WXPlayerSrcName(@"nonfullscreen")] ?: [UIImage imageNamed:WXPlayerFrameworkSrcName(@"nonfullscreen")] forState:UIControlStateSelected];
...
//        
//    }];
    self.fullScreenBtn.frame = CGRectMake(xScreenWidth-40, 0, 40, 40);
...
//        make.bottom.equalTo(self.bottomView).with.offset(0);
//    }];
    self.leftTimeLabel.frame = CGRectMake(45, 20, xScreenWidth-90, 20);
...
//        make.bottom.equalTo(self.bottomView).with.offset(0);
//    }];
    self.rightTimeLabel.frame = CGRectMake(45, 20, xScreenWidth-90, 20);
...
//
//    }];
    self.titleLabel.frame = CGRectMake(45, 0, xScreenWidth-90, 40);
...
//
//        }];
        self.loadFailedLabel.frame = CGRectMake(0, 0, xScreenWidth, 30);
    }
...
- (void)appDidEnterBackground:(NSNotification*)note
{
    if (self.playOrPauseBtn.isSelected==NO) {//如果是播放中，则继续播放
        ...
        [self.player play];
        self.state = WXPlayerStatePlaying;
    }else{
        self.state = WXPlayerStateStopped;
    }
}
#pragma mark 
#pragma mark 进入前台
...
//    self.toSwiftFullBlock(sender);
    if (self.delegate&&[self.delegate respondsToSelector:@selector(wxplayer:clickedFullScreenButton:)]) {
        [self.delegate wxplayer:self clickedFullScreenButton:sender];
    }
...
//    self.toSwiftCloseBlock(sender);
    if (self.delegate&&[self.delegate respondsToSelector:@selector(wxplayer:clickedCloseButton:)]) {
        [self.delegate wxplayer:self clickedCloseButton:sender];
    }
}
///获取视频长度
- (double)duration{
...
- (void)PlayOrPause:(UIButton *)sender{
    ...
    if ([self.delegate respondsToSelector:@selector(wxplayer:clickedPlayOrPauseButton:)]) {
        [self.delegate wxplayer:self clickedPlayOrPauseButton:sender];
    }
}
///播放
-(void)play{
...
- (void)handleSingleTap:(UITapGestureRecognizer *)sender{
    ...
    if (self.delegate&&[self.delegate respondsToSelector:@selector(wxplayer:singleTaped:)]) {
        [self.delegate wxplayer:self singleTaped:sender];
    }
...
- (void)handleDoubleTap:(UITapGestureRecognizer *)doubleTap{
    if (self.delegate&&[self.delegate respondsToSelector:@selector(wxplayer:doubleTaped:)]) {
        [self.delegate wxplayer:self doubleTaped:doubleTap];
    }
    if (self.player.rate != 1.f) {
        if ([self currentTime] == self.duration)
            [self setCurrentTime:0.f];
...
- (void)setURLString:(NSString *)URLString{
    ...
    self.playerLayer.videoGravity = AVLayerVideoGravityResize;
    [self.layer insertSublayer:_playerLayer atIndex:0];
    self.state = WXPlayerStateBuffering;
    if (self.closeBtnStyle==CloseBtnStylePop) {
        [_closeBtn setImage:[UIImage imageNamed:WXPlayerSrcName(@"play_back.png")] ?: [UIImage imageNamed:WXPlayerFrameworkSrcName(@"play_back.png")] forState:UIControlStateNormal];
        [_closeBtn setImage:[UIImage imageNamed:WXPlayerSrcName(@"play_back.png")] ?: [UIImage imageNamed:WXPlayerFrameworkSrcName(@"play_back.png")] forState:UIControlStateSelected];
        
    }else{
        [_closeBtn setImage:[UIImage imageNamed:WXPlayerSrcName(@"close")] ?: [UIImage imageNamed:WXPlayerFrameworkSrcName(@"close")] forState:UIControlStateNormal];
        [_closeBtn setImage:[UIImage imageNamed:WXPlayerSrcName(@"close")] ?: [UIImage imageNamed:WXPlayerFrameworkSrcName(@"close")] forState:UIControlStateSelected];
    }
}
 ...
 */
- (void)setState:(WXPlayerState)state
{
    _state = state;
    // 控制菊花显示、隐藏
    if (state == WXPlayerStateBuffering) {
        [self.loadingView startAnimating];
    }else if(state == WXPlayerStatePlaying){
        [self.loadingView stopAnimating];//
    }else if(state == WXPlayerStatusReadyToPlay){
        [self.loadingView stopAnimating];//
    }
    else{
        [self.loadingView stopAnimating];//
...
- (void)moviePlayDidEnd:(NSNotification *)notification {
    self.state            = WXPlayerStateFinished;
    if (self.delegate&&[self.delegate respondsToSelector:@selector(wxplayerFinishedPlay:)]) {
        [self.delegate wxplayerFinishedPlay:self];
    }
    ...
    if (context == PlayViewStatusObservationContext)
    {
        if ([keyPath isEqualToString:@"status"]) {
            ...
            switch (status)
            {
                ...
                {
                    self.state = WXPlayerStatusReadyToPlay;
                    // 双击的 Recognizer
...
//                    }
                ...
                {
                    self.state = WXPlayerStateFailed;
                    if (self.delegate&&[self.delegate respondsToSelector:@selector(wxplayerFailedPlay:WXPlayerStatus:)]) {
                        [self.delegate wxplayerFailedPlay:self WXPlayerStatus:WXPlayerStateFailed];
                    }
            ...
            [self.loadingView stopAnimating];
            // 当缓冲好的时候
            if (self.currentItem.playbackLikelyToKeepUp && self.state == WXPlayerStateBuffering){
                self.state = WXPlayerStatePlaying;
...
- (void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event{
    ...
    if (verValue > horValue) {//上下滑动
        ...
        if (self.isFullscreen) {//全屏下
            ...
            if (self.originalPoint.x <= xHalfHeight) {//全屏下:point在view的左边(控制音量)
            ...
            //判断刚开始的点是左边还是右边,左边控制音量
            if (self.originalPoint.x <= xHalfWidth) {//非全屏下:point在view的左边(控制音量)
